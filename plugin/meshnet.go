...
func (pod *podMeta) getPodMetadata(ctx context.Context, kv clientv3.KV) error {
	...
	if err != nil {
		...
		return fmt.Errorf("Could not find `src_ip` key for pod %s", pod.Name)
	}
...
// Creates (koko) api.Veth from NetNS and LinkName
func makeVeth(netNS, linkName string, ip string) (*api.VEth, error) {
...
// Creates (koko) api.Vxlan from ParentIF, destination IP and VNI
func makeVxlan(srcIntf string, peerIP string, idx int) *api.VxLan {
	return &api.VxLan{
		ParentIF: srcIntf,
...
func cmdAdd(args *skel.CmdArgs) error {
	...
	log.Printf("Processing POD %s", cniArgs.K8S_POD_NAME)
